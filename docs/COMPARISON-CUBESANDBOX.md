...
1. **Policy generations in egress enforcement**: stamp a generation into our iptables chains (chain name/comment + conntrack flush on bump) so policy changes and snapshot restores invalidate established flows. Maps our epoch semantics into the datapath. (`CubeNet tap.go BumpMvmVersion`)
...
3. **Agent-on-pmem delivery**: ship guest-supervisor as a separate read-only ext4 on a second virtio device mounted by guest init; upgrades decouple from rootfs builds. (`guest-init/`, ~200 lines)
4. **Self-describing snapshot packages**: snapshot dir carries the full recreate payload (workspace generation, epoch, egress policy, supervisor/kernel versions) — enables cross-node restore later; their version matrix flags stale templates. (`pause_package.go`, `snapshot/mod.rs`)
5. **Atomic fail-closed iptables updates**: build scratch chain → verify with `-C` → swap. (`cube-proxy-iptables-init.sh`)

**P1 — architectural upgrades:**
6. **Incremental memory snapshots**: Firecracker already supports dirty-page tracking; their soft-dirty (`clear_refs`+pagemap bit 55) and pagemap_anon fallback with range coalescing is the reference implementation. Big checkpoint-economics win for M8 economics. (`hypervisor/vmm/src/soft_dirty.rs`)
7. **Restore placement guards**: `cpuid_hash` + host kernel release must match for cross-host snapshot restore; origin-node-first preference in the scheduler. (`restoreplace/placement.go`)
...
10. **WAL discipline audit** of our durable stores against their four rules: one batch in flight, ack after fsync, CRC'd batch-close, torn-tail-discard-whole; plus recovery-time-bound checkpoint interval for outbox compaction.
